<?php
// MODIGO - API GPS chauffeur
// Compatible avec :
// 1) une session utilisateur active ;
// 2) un accès chauffeur par token/QR Code.
// Cette page répond toujours en JSON.
...
try {
    ...
    // Ne pas charger auth.php ici : une redirection HTML casserait la réponse JSON.
    ...
    // Le token peut arriver dans le JSON, en POST ou dans l'URL.
    ...
    // Priorité à la session lorsqu'elle est complète et autorisée.
    ...
    // Si la session n'est pas utilisable, identification du chauffeur par token.
    if ($authMode === '') {
        if ($token === '') {
            json_response([
                'ok' => false,
                'message' => 'Accès GPS non authentifié. Reconnectez-vous ou utilisez le lien chauffeur.'
            ], 401);
        }
        ...
        if (!$chauffeur) {
            json_response([
                'ok' => false,
                'message' => 'Lien chauffeur invalide ou expiré.'
            ], 401);
        }

        $chauffeurId = (int) $chauffeur['id'];
        $societeId = (int) $chauffeur['societe_id'];
        $role = (string) $chauffeur['role'];
        $authMode = 'token';
    }

    if ($chauffeurId <= 0 || $societeId <= 0) {
        json_response([
            'ok' => false,
            'message' => 'Compte chauffeur incomplet.'
        ], 403);
    }
    ...
    if ($action === 'service') {
        ...
        if (!in_array($status, ['hors_service', 'en_service', 'pause'], true)) {
            json_response([
                'ok' => false,
                'message' => 'Statut de service invalide.'
            ], 422);
        }
    ...
    $battery = is_numeric($input['battery'] ?? null)
    ...
    // Mise à jour de la mission réellement affichée sur le téléphone.
    // La société et le chauffeur sont toujours vérifiés pour empêcher toute modification étrangère.
    $courseUpdated = false;
    ...
    if ($courseId !== false && $courseId !== null) {
        ...
        $stmt->execute([$lat, $lng, $courseId, $societeId, $chauffeurId]);
        $courseUpdated = $stmt->rowCount() > 0;
    } else {
        // Compatibilité avec les anciennes versions qui n'envoient pas course_id.
        $stmt = $pdo->prepare(
            "UPDATE courses
             SET latitude = ?, longitude = ?
             WHERE societe_id = ?
    ...
    json_response([
        'ok' => true,
        'auth_mode' => $authMode,
        'course_id' => $courseId !== false ? $courseId : null,
    ...
    json_response([
        'ok' => false,
        'message' => 'Erreur serveur GPS. Consultez le journal PHP.'
    ], 500);
}
