...
function regulation_courses(PDO $pdo, int $societeId): array
{
    $sql = "SELECT c.*, u.prenom AS chauffeur_nom
            FROM courses c
            LEFT JOIN users u ON u.id = c.chauffeur_id
            WHERE c.societe_id = ?
              AND c.statut NOT IN ('terminée','terminee','terminé','termine','TERMINEE','TERMINE')
            ORDER BY
              CASE WHEN c.date_course = CURDATE() THEN 0 ELSE 1 END,
              c.date_course ASC,
              c.heure_pickup ASC";
    $st = $pdo->prepare($sql);
    $st->execute([$societeId]);
...
function regulation_course_payload(array $courses): array
{
    $byDriver = [];
    $waiting = 0;
    $running = 0;
    ...
    foreach ($courses as $c) {
        $statusRaw = strtolower(trim((string)($c['statut'] ?? 'prévue')));
        $isRunning = in_array($statusRaw, ['en cours', 'en_cours', 'encours'], true);
        $isWaiting = !$isRunning;
        if ($isRunning) $running++; else $waiting++;

        $plannedTs = 0;
        if (!empty($c['date_course']) && !empty($c['heure_pickup'])) {
            $plannedTs = (int)strtotime((string)$c['date_course'].' '.(string)$c['heure_pickup']);
        }
        $isLate = $plannedTs > 0 && $plannedTs < $now && !$isRunning && empty($c['depart_reel']);
        if ($isLate) $late++;

        $driverId = (int)($c['chauffeur_id'] ?? 0);
        if ($driverId > 0 && !isset($byDriver[$driverId])) {
            $byDriver[$driverId] = [
                'id' => (int)($c['id'] ?? 0),
                'patient' => trim((string)($c['client_nom'] ?? $c['nom_patient'] ?? '')) ?: 'Patient non renseigné',
                'date' => (string)($c['date_course'] ?? ''),
                'time' => !empty($c['heure_pickup']) ? substr((string)$c['heure_pickup'], 0, 5) : '--:--',
                'departure' => trim((string)($c['adresse_depart'] ?? '')) ?: 'Départ non renseigné',
                'destination' => trim((string)($c['adresse_arrivee'] ?? '')) ?: 'Destination non renseignée',
                'status' => $isRunning ? 'En cours' : 'Prévue',
                'late' => $isLate,
                'plannedTs' => $plannedTs,
            ];
        }
    ...
    return [
        'byDriver' => $byDriver,
        'stats' => ['waiting' => $waiting, 'running' => $running, 'late' => $late, 'completedToday' => $completedToday],
    ];
}

function gps_payload(array $rows, array $courseData = []): array
{
    $now = time();
    $drivers = [];
    $courseStats = $courseData['stats'] ?? ['waiting'=>0,'running'=>0,'late'=>0,'completedToday'=>0];
    $courseByDriver = $courseData['byDriver'] ?? [];
    $stats = ['connected' => 0, 'in_service' => 0, 'available' => 0, 'alerts' => 0, 'total' => count($rows), 'waiting' => (int)$courseStats['waiting'], 'running' => (int)$courseStats['running'], 'late' => (int)$courseStats['late']];
    ...
    foreach ($rows as $r) {
        ...
        $alerts = [];
        if (!$hasPosition) {
            $alerts[] = ['type' => 'warning', 'label' => 'Position GPS en attente'];
        } elseif (!$connected) {
            $alerts[] = ['type' => 'danger', 'label' => 'Signal GPS ancien'];
        }
        ...
        if (!$hasPosition && $status === 'en_service') {
            $displayStatus = 'En service · GPS en attente';
            $color = 'amber';
        } elseif ($connected && $status === 'en_service') {
            $displayStatus = 'En service';
            $color = 'green';
        } elseif ($connected && $status === 'pause') {
            $displayStatus = 'Pause';
            $color = 'purple';
        } elseif ($connected) {
            $displayStatus = 'Disponible';
            $color = 'blue';
        } elseif ($hasPosition && $age <= 900) {
            $displayStatus = 'Signal GPS ancien';
            $color = 'amber';
        }
        ...
        $drivers[] = [
            ...
            'alerts' => $alerts,
            'mission' => $courseByDriver[(int)($r['chauffeur_id'] ?? 0)] ?? null,
        ];
    }
...
if (isset($_GET['ajax'])) {
    ...
    try {
        echo json_encode(gps_payload(gps_rows($pdo, $societeId), regulation_course_payload(regulation_courses($pdo, $societeId))), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    } catch (Throwable $e) {
...
$initial = ['ok' => true, 'generatedAt' => date('H:i:s'), 'stats' => ['connected'=>0,'in_service'=>0,'available'=>0,'alerts'=>0,'total'=>0,'waiting'=>0,'running'=>0,'late'=>0], 'drivers' => []];
$gpsError = '';
try { $initial = gps_payload(gps_rows($pdo, $societeId), regulation_course_payload(regulation_courses($pdo, $societeId))); }
...
*{box-sizing:border-box}.supervision-shell{min-height:calc(100vh - 70px);background:linear-gradient(145deg,#06111f,#081728);color:var(--text);padding:14px;font-family:Arial,sans-serif}.sup-header{display:flex;align-items:center;gap:16px;margin-bottom:12px}.sup-brand{min-width:240px}.sup-brand h1{font-size:23px;margin:0}.sup-brand small{color:#54a3ff;font-weight:700}.sup-kpis{display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:10px;flex:1}.kpi{background:linear-gradient(145deg,var(--panel),var(--panel2));border:1px solid var(--line);border-radius:13px;padding:12px 14px}.kpi-label{font-size:11px;color:var(--muted);text-transform:uppercase}.kpi-value{font-size:25px;font-weight:800;margin-top:5px}.kpi small{color:var(--muted)}.sup-main{display:grid;grid-template-columns:270px minmax(420px,1fr) 330px;gap:12px;height:calc(100vh - 250px);min-height:560px}.panel{background:rgba(9,25,42,.97);border:1px solid var(--line);border-radius:13px;overflow:hidden}.panel-title{padding:14px;border-bottom:1px solid var(--line);font-size:13px;font-weight:800;text-transform:uppercase}.driver-tools{padding:10px}.driver-tools input{width:100%;background:#071523;border:1px solid var(--line);color:white;border-radius:8px;padding:10px}.driver-list{height:calc(100% - 96px);overflow:auto;padding:0 8px 10px}.driver-item{display:block;width:100%;text-align:left;background:#0b1d30;border:1px solid #18334d;border-radius:10px;padding:11px;margin:7px 0;color:white;cursor:pointer}.driver-item:hover,.driver-item.active{border-color:#2a8cff;background:#102945}.driver-line{display:flex;justify-content:space-between;gap:6px}.driver-name{font-weight:800}.driver-meta{font-size:12px;color:var(--muted);margin-top:4px}.status{display:inline-flex;align-items:center;gap:5px;font-size:11px;margin-top:7px}.dot{width:8px;height:8px;border-radius:50%}.green{background:var(--green)}.blue{background:var(--blue)}.red{background:var(--red)}.purple{background:var(--purple)}.amber{background:var(--amber)}.map-panel{position:relative}.map-wrap,#map{width:100%;height:100%}.sync-badge{position:absolute;z-index:500;top:10px;right:10px;background:#071523dd;border:1px solid var(--line);padding:8px 11px;border-radius:9px;font-size:12px}.detail-content{padding:16px;overflow:auto;height:calc(100% - 47px)}.driver-hero{display:flex;gap:12px;align-items:center;padding-bottom:15px;border-bottom:1px solid var(--line)}.vehicle-icon{width:56px;height:56px;border-radius:50%;display:grid;place-items:center;font-size:27px;background:#173653}.detail-row{display:flex;justify-content:space-between;gap:12px;padding:12px 0;border-bottom:1px solid #17314a;font-size:13px}.detail-row span:first-child{color:var(--muted)}.center-btn{width:100%;border:0;background:#1769d2;color:white;border-radius:8px;padding:12px;font-weight:700;margin-top:15px;cursor:pointer}.alerts-bar{margin-top:12px;background:var(--panel);border:1px solid var(--line);border-radius:13px;padding:10px 14px;display:flex;gap:10px;align-items:center;min-height:58px}.alert-chip{padding:8px 11px;border-radius:8px;font-size:12px;background:#2a1820;border:1px solid #73303c}.mission-box{margin-top:14px;padding:12px;border:1px solid #214564;border-radius:10px;background:#0a1a2b}.mission-title{font-weight:800;margin-bottom:8px}.mission-line{font-size:12px;color:#b7c5d6;margin:7px 0;line-height:1.35}.mission-late{color:#ffb4b4;font-weight:800}.journal{margin-top:12px;background:var(--panel);border:1px solid var(--line);border-radius:13px;overflow:hidden}.journal-head{padding:10px 14px;border-bottom:1px solid var(--line);font-weight:800}.journal-list{display:grid;grid-template-columns:repeat(4,minmax(180px,1fr));gap:8px;padding:10px;overflow:auto}.journal-item{background:#0b1d30;border:1px solid #18334d;border-radius:9px;padding:9px;font-size:12px}.journal-time{color:#54a3ff;font-weight:800;margin-bottom:4px}.empty{padding:20px;color:var(--muted);text-align:center}.leaflet-popup-content-wrapper,.leaflet-popup-tip{background:#0b1b2d;color:white}.vehicle-marker{display:grid;place-items:center;width:38px;height:38px;border:3px solid white;border-radius:50%;box-shadow:0 4px 12px #0008;font-size:19px}.marker-green{background:var(--green)}.marker-blue{background:var(--blue)}.marker-red{background:var(--red)}.marker-purple{background:var(--purple)}.marker-amber{background:var(--amber)}
...
<div class="supervision-shell">
  <header class="sup-header">
    ...
    <div class="sup-kpis">
      ...
      <div class="kpi"><div class="kpi-label">Courses en attente</div><div class="kpi-value" id="kpiWaiting">0</div><small>à affecter ou démarrer</small></div>
      <div class="kpi"><div class="kpi-label">Courses en cours</div><div class="kpi-value" id="kpiRunning">0</div><small>missions actives</small></div>
      <div class="kpi"><div class="kpi-label">Retards potentiels</div><div class="kpi-value" id="kpiLate">0</div><small>à contrôler</small></div>
  ...
  <section class="journal"><div class="journal-head">🕘 Journal d’activité en direct</div><div id="journalList" class="journal-list"></div></section>
</div>
...
function render(data){
 ...
 document.getElementById('kpiWaiting').textContent=s.waiting||0;
 document.getElementById('kpiRunning').textContent=s.running||0;
 document.getElementById('kpiLate').textContent=s.late||0;
 ...
 document.getElementById('driverList').innerHTML=visible.length?visible.map(d=>`<button class="driver-item ${selectedId===d.id?'active':''}" onclick="selectDriver(${d.id})"><div class="driver-line"><span class="driver-name">${esc(d.name)}</span><span>${d.battery===null?'--':d.battery+'%'}</span></div><div class="driver-meta">${esc(d.vehicle)}${d.mission?' · '+esc(d.mission.time)+' '+esc(d.mission.patient):''}</div><div class="status"><i class="dot ${d.color}"></i>${esc(d.status)} · ${ageText(d.age)}</div></button>`).join(''):'<div class="empty">Aucun chauffeur trouvé</div>';
 ...
 const journal=[]; data.drivers.forEach(d=>{journal.push({t:data.generatedAt,txt:d.name+' : position GPS reçue'});if(d.mission)journal.push({t:d.mission.time,txt:d.name+' : '+d.mission.status+' · '+d.mission.patient});});document.getElementById('journalList').innerHTML=journal.slice(0,8).map(x=>`<div class="journal-item"><div class="journal-time">${esc(x.t)}</div>${esc(x.txt)}</div>`).join('')||'<div class="empty">Aucune activité récente</div>';
 if(selectedId) showDetail(selectedId);
}
function showDetail(id){
 const d=currentData.drivers.find(x=>x.id===id);if(!d)return;
 const mission=d.mission?`<div class="mission-box"><div class="mission-title">🚗 Mission actuelle / prochaine</div><div class="mission-line"><b>${esc(d.mission.patient)}</b></div><div class="mission-line">🕒 ${esc(d.mission.time)} · ${esc(d.mission.status)} ${d.mission.late?'<span class="mission-late">· RETARD POTENTIEL</span>':''}</div><div class="mission-line">📍 ${esc(d.mission.departure)}</div><div class="mission-line">🏥 ${esc(d.mission.destination)}</div><a class="center-btn" style="display:block;text-align:center;text-decoration:none" href="edit_course.php?id=${d.mission.id}">Ouvrir la course</a></div>`:`<div class="mission-box"><div class="mission-title">✅ Aucune mission active</div><div class="mission-line">Le chauffeur est disponible pour une affectation.</div></div>`;
 document.getElementById('driverDetail').innerHTML=`<div class="driver-hero"><div class="vehicle-icon">🚑</div><div><h2 style="margin:0 0 4px">${esc(d.name)}</h2><div style="color:#91a4ba">${esc(d.vehicle)}</div><div class="status"><i class="dot ${d.color}"></i>${esc(d.status)}</div></div></div><div class="detail-row"><span>Téléphone</span><b>${esc(d.phone)}</b></div><div class="detail-row"><span>Batterie</span><b>${d.battery===null?'--':d.battery+' %'}</b></div><div class="detail-row"><span>Qualité GPS</span><b>${d.accuracy===null?'--':d.accuracy+' m'}</b></div><div class="detail-row"><span>Vitesse</span><b>${d.speed===null?'--':d.speed+' km/h'}</b></div><div class="detail-row"><span>Réseau</span><b>${esc(d.network)}</b></div><div class="detail-row"><span>Dernière transmission</span><b>${ageText(d.age)}</b></div>${mission}<button class="center-btn" onclick="centerDriver(${d.id})">⊙ Centrer sur la carte</button>`;
}
